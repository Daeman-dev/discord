...
# ID роли "День рождения"
birthday_role_id = 998877665544332211  # Замените на ID роли "День рождения"
channel_id = 706224150704029697  # Замените на ID вашего канала
users_to_mention = [294860783811035136, 294881060263821312,135024441884278784,904383863806128189]  # Замените на ID пользователей
...
# # Команда для добавления дня рождения
# @bot.slash_command(
#     name="добавитьдр",
...
#     user: Option(discord.Member, "Пользователь, чей день рождения добавляем."),
#     date: Option(str, "Дата рождения в формате ДД-ММ.")
# ):
#     """
#     Добавляет день рождения пользователя в базу данных.
#     """
#     with sqlite3.connect(DATABASE_FILE) as conn:
#         cursor = conn.cursor()
#         cursor.execute("INSERT OR REPLACE INTO birthdays (user_id, birthday) VALUES (?, ?)", (user.id, date))
#         conn.commit()
#     await ctx.respond(f"День рождения пользователя {user.name} добавлен: {date}")
